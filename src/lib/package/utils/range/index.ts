 ...
 *
 * The main function exported by this module is `range`, which takes
 ...
 */
export function interpolate(params: CurveParams, value: number): number {
 ...
 */
export default function range(ranges: Range[]): Color[] {
  const colors: Color[] = [];
  ...
  for (let j = 0; j < ranges.length; j++) {
    ...
    for (let i = j; i < count; i++) {
      ...
      color.h = interpolate(hueCurve, color.h);
      color.s = interpolate(saturationCurve, color.s);
      color.l = interpolate(lightnessCurve, color.l);
